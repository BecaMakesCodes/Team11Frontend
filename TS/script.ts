...
function TombgGenerator(meret:number, alsoHatar:number, felsoHatar:number):number[]{
let array:number[] = [];
for(let i:number = 0; i<meret; i++){
    let random:number = Math.floor(Math.random() * (felsoHatar - alsoHatar + 1)) + alsoHatar;
    array.push(random);
}
return array;
}
